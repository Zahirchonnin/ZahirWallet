...
class ZahirWallet(titleBar):
    ...
    def _addAccount(self, prvKey, acc_name):
        ...
        try:
            ...
            balance = self.w3.fromWei(self.w3.eth.get_balance(self.account.address), 'ether')
            self.Main.balance.setText(str(balance) + ' ETH')
    ...
    def sendCoin(self):
        if self.prvKey:
            nonce = self.w3.eth.getTransactionCount(self.Main.account_address.text())
            _from = self.Main.account_address.text()
            _to = self.Send.To.text()
            amount = self.Send.amount.value()
            if amount > self.w3.fromWei(self.w3.eth.get_balance(self.Main.account_address.text()), 'ether'):
                return self.Send.label.setText('insufficient balance!')
            ...
            try:
                ...
                self.contanier.setCurrentIndex(self.widgets['Main'])
                self.Main.account_address.setText(self.account.address)
                self.Main.balance.setText(str(self.w3.fromWei(
                    self.w3.eth.get_balance(self.account.address
                    ), 'ether')) + ' ETH')
            
            except Exception as e:
                self.Send.label.setText('Invalid input!')

        else:
            self.Send.label.setText('No account selected!')
    ...
    def applyMethod(self):
        ...
        _value = self.w3.toWei(self.Excutor._value.value(), 'ether')
        method = self.Excutor.methods.currentText()
        ...
        self.Main.account_address.setText(self.account.address)
        self.Main.balance.setText(str(self.w3.eth.get_balance(self.account.address)) + ' ETH')
    ...
    def changeRPC(self):
        ...
        if self.w3.isConnected():
            self.rpc = rpc
            balance = self.w3.fromWei(self.w3.eth.get_balance(self.Main.account_address.text()), 'ether')
            self.Main.balance.setText(str(balance) + ' ETH')
            self.frame.hide()
